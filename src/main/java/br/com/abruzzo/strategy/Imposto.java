/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

package br.com.abruzzo.strategy;

/**
 *
 * @author Emmanuel de Oliveira D'Abruzzo
 * @data 19 de jul. de 2020
 */
abstract class Imposto {
    
    Imposto impostoDecorator;

    public Imposto() {
    }
    
    

    public Imposto(Imposto impostoDecorator) {
        this.impostoDecorator = impostoDecorator;
    }
    
    
    
    double calcular(Orcamento orcamento, double aliquota){
        
        
        if(this.impostoDecorator != null)
            return orcamento.getValor()*aliquota + this.impostoDecorator.calcular(orcamento, 0);
        else
            return orcamento.getValor() * aliquota;
    }

}
